  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("attendance").upsert(records, {
        onConflict: ["student_id", "attendance_date"], // make sure unique constraint exists
      });

      if (error) throw error;

      await logActivity("Marked daily attendance");
      toast.success("Attendance saved successfully!"); // ✅ toast notification
    } catch (err) {
  ...
  const handleAssignStudent = async () => {
    ...
    try {
      ...
      const { data: existing } = await supabase
        ...
        .select("*")
        .eq("student_id", assignForm.studentId)
        .eq("course_id", assignForm.courseId);

      if (existing?.length) {
        toast.error(
          `${student.full_name} is already assigned to ${course.name}`
        );
        return;
      }
      ...
      toast.success(`Assigned ${student.full_name} to ${course.name}!`);
